...
describe('getClassificationsByCategories', () => {
  ...
  it('単一カテゴリーでフィルタリングする', () => {
    const result = getClassificationsByCategories(mockFishData, ['マグロの仲間']);
    expect(result).toEqual(['サバ科']);
  });

  it('複数カテゴリーでフィルタリングする', () => {
    const result = getClassificationsByCategories(mockFishData, ['マグロの仲間', 'フグの仲間']);
    expect(result).toEqual(['サバ科', 'フグ科']);
  });

  it('該当なしの場合は空配列を返す', () => {
    const result = getClassificationsByCategories(mockFishData, ['存在しないカテゴリー']);
...
describe('filterFishData', () => {
  ...
  it('複合フィルタリング（カテゴリー + 分類）', () => {
    const result = filterFishData(mockFishData, ['マグロの仲間'], ['サバ科'], []);
    expect(result).toHaveLength(2);
    expect(result.every(f => f.category === 'マグロの仲間' && f.classification === 'サバ科')).toBe(true);
  });

  it('複合フィルタリング（カテゴリー + 分類 + レア度）', () => {
    const result = filterFishData(mockFishData, ['マグロの仲間'], ['サバ科'], [3]);
    expect(result).toHaveLength(1);
    expect(result[0].name).toBe('マグロ');
  });

  it('該当なしの場合は空配列を返す', () => {
    const result = filterFishData(mockFishData, ['存在しないカテゴリー'], [], []);
    expect(result).toEqual([]);
  });
  ...
  it('複数カテゴリーでフィルタリングする', () => {
    const result = filterFishData(mockFishData, ['マグロの仲間', 'タイの仲間'], [], []);
