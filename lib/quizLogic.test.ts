...
describe('saveRetryData', () => {
  ...
  it('sessionStorageに正しくデータを保存する', () => {
    ...
    const settings = { categories: ['マグロの仲間'], classifications: ['サバ科'], rarities: [3] };
  ...
  it('RetryQuizData構造が正しい', () => {
    ...
    const settings = { categories: ['フグの仲間'], classifications: [], rarities: [] };
  ...
  it('sessionStorage.setItem()エラーを処理する', () => {
    ...
    // エラーが発生してもクラッシュしない
    expect(() => {
      saveRetryData([mockFishData[0]], { categories: [], classifications: [], rarities: [] }, 0, 1);
    }).not.toThrow();
...
describe('loadRetryData', () => {
  ...
  it('不正な構造の場合はnullを返してクリアする', () => {
    // modeが欠けている
    sessionStorage.setItem('quiz-retry-data', JSON.stringify({ questions: [] }));
  ...
  it('questionsがない場合もnullを返す', () => {
    sessionStorage.setItem('quiz-retry-data', JSON.stringify({ mode: 'retry' }));
...
describe('clearRetryData', () => {
  ...
  it('sessionStorageからデータを削除する', () => {
    sessionStorage.setItem('quiz-retry-data', JSON.stringify({ mode: 'retry', questions: [] }));
